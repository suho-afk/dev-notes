...
/**
* 기존 MyString 클래스에서
* assing,reserve,capacity,at,insert,erase,find,compare 추가
* KMP,Boyer - Moore 알고리즘을 추가한 find
*/
#include <string.h>

std::vector<int> getPartialMatch(const MyString& pattern) {
...
std::vector<int> buildBadCharTable(const MyString& pattern) {
	std::vector<int> badChar(256, -1); // ASCII 기준
	for (int i = 0; i < pattern.length(); i++) {
		badChar[(unsigned char)pattern.at(i)] = i;
	}
	return badChar;
}
...
public:
	...
	int findKMP(int find_from, const MyString& pattern) const;
	int findKMP(int find_from, const char* str) const;
	int findKMP(int find_from, char c) const;
	int findBM(int find_from, const MyString& pattern) const;
...
int MyString::findKMP(int find_from, const MyString& pattern) const
{
	if (pattern.length() == 0) return -1;
	std::vector<int> pi = getPartialMatch(pattern);
	int j = 0;

	for (int i = find_from; i < this->length(); i++) {
		while (j > 0 && this->at(i) != pattern.at(j)) {
			j = pi[j - 1];
		}
		if (this->at(i) == pattern.at(j)) {
			j++;
			if (j == pattern.length()) {
				return i - pattern.length() + 1; // 매칭 시작 위치 반환
			}
		}
	}
	return -1; // 찾지 못함
...
int MyString::findBM(int find_from, const MyString& pattern) const
{
	if (pattern.length() == 0) return -1;
	std::vector<int> badChar = buildBadCharTable(pattern);
	int n = this->length();
	int m = pattern.length();
	int s = find_from; // shift 위치

	while (s <= n - m) {
		int j = m - 1;

		// 뒤에서부터 비교
		while (j >= 0 && pattern.at(j) == this->at(s + j)) {
			j--;
		}
		...
		if (j < 0) {
			...
			// 불일치: bad character rule에 따라 점프
			int bad_char_index = (unsigned char)this->at(s + j);
			int shift = std::max(1, j - badChar[bad_char_index]);
			s += shift;
		}
	}
	return -1;
}
int MyString::findBM(int find_from, const char* str) const
{
	MyString temp(str);
	return findBM(find_from, temp);
}
int MyString::findBM(int find_from, char c) const
{
	MyString temp(c);
	return findBM(find_from, temp);
}
